...
def binary_class_split(dataset, label_name):
    ...
    for i in range(len(dataset)):
        label_value = dataset[i][label_name] 
        
        if label_value not in class_datasets:
            class_datasets[label_value] = []
        
        class_datasets[label_value].append(dataset[i])
...
def get_0_1_index(df, labels):
    ...
    for label in unique_labels:
        label_indices[label] = [i for i, x in enumerate(df[labels]) if x == label]
...
def build_dataset(dataset_smiles, label_name, smiles_name, descriptor_seq, is_descriptor=True):
    ...
    desciptor = dataset_smiles[descriptor_seq]  
...
def multi_task_build_dataset(dataset_smiles, labels_list, smiles_name, descriptor_seq, is_descriptor=True):  
    dataset_gnn = []
    failed_molecule = []
    labels = dataset_smiles[labels_list]
    split_index = dataset_smiles['group']
    smilesList = dataset_smiles[smiles_name]
    descriptor = dataset_smiles[descriptor_seq].to_numpy()
    ...
    for i, smiles in enumerate(smilesList):
        try:
            ...
            molecule = [smiles, g, labels.loc[i], mask, split_index.loc[i], dataset_smiles.loc[i][descriptor_seq]]    
            dataset_gnn.append(molecule)
...
def built_data_and_save_for_splited(
    ...
    smiles_name = 'smiles'
    descriptor_seq = data_origin.columns[6:]    
    data_origin = data_origin.fillna(123456)      
    labels_list = [x for x in data_origin.columns if x not in ['smiles', 'group'] + list(descriptor_seq)]   
    if task_list_selected is not None:
        labels_list = task_list_selected
    data_set_gnn = multi_task_build_dataset(dataset_smiles=data_origin, labels_list=labels_list, smiles_name=smiles_name, descriptor_seq = descriptor_seq)

    smiles, graphs, labels, mask, split_index, descriptor = map(list, zip(*data_set_gnn)) 
    graph_labels = {'labels': torch.tensor(labels),
               'mask': torch.tensor(mask)
               }
    split_index_pd = pd.DataFrame(columns=['smiles', 'group', 'descriptor'])
    split_index_pd.smiles = smiles
 #   print(split_index_pd.smiles)
    split_index_pd.group = split_index
    np.set_printoptions(threshold=sys.maxsize)
    descriptor_values = pd.DataFrame(descriptor).values
  #  print(f"Shape of descriptor_values: {descriptor_values.shape}")
    split_index_pd['descriptor'] = descriptor_values.tolist() 
    
  #  print(split_index_pd['descriptor'])
   # print(f"Shape of descriptor_split: {split_index_pd.descriptor.shape}")
...
def load_graph_from_csv_bin_for_splited(
    ...
    descriptor = pd.read_csv(group_path, index_col=None).descriptor.values
  ...
  #  descriptor = detailed_information['descriptor']
   # print(descriptor)
    ...
    train_descriptor, val_descriptor, test_descriptor = split_dataset_according_index(descriptor, train_index, val_index, test_index)
    ...
    for i in range(len(train_index)):
        molecule = [train_smiles[i], train_graph[i], train_descriptor[i], train_labels.values[i], train_mask.values[i]]
        train_set.append(molecule)

    for i in range(len(val_index)):
        molecule = [val_smiles[i], val_graph[i], val_descriptor[i], val_labels.values[i], val_mask.values[i]]
        val_set.append(molecule)

    for i in range(len(test_index)):
        molecule = [test_smiles[i], test_graph[i], test_descriptor[i], test_labels.values[i], test_mask.values[i]]
